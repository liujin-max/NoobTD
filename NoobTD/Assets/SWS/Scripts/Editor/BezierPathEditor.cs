        ...
        void OnSceneGUI()
        {
            ...
            for (int i = 0; i < script.bPoints.Count; i++)
            {
                ...
                if (i > 0)
                {
                    ...
                    moved = Handles.FreeMoveHandle(point.cp[0].position, Quaternion.identity, controlSize, Vector3.zero, Handles.SphereHandleCap);
                ...
                if (i < script.bPoints.Count - 1)
                {
                    ...
                    moved = Handles.FreeMoveHandle(point.cp[1].position, Quaternion.identity, controlSize, Vector3.zero, Handles.SphereHandleCap);
            ...
            for (int i = 0; i < pathPoints.Length; i++)
                ...
                Mathf.Clamp((HandleUtility.GetHandleSize(pathPoints[i]) * 0.12f), 0, 0.25f), new EventType());
            ...
            {   
                ...
                for(int j = 0; j < segments[i].Count; j++)
                {
                    ...
                    Handles.ArrowHandleCap( 0, segments[i][j], Quaternion.Lerp(script.bPoints[i].wp.rotation, script.bPoints[i+1].wp.rotation, lerpVal) * Quaternion.Euler( 0, 90, 0 ), size, new EventType());
                }
            }
        }
